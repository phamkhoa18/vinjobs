...
export default function RegisterPage() {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        setLoading(true);
        setError('');
        await googleLogin(tokenResponse.access_token || tokenResponse.credential || tokenResponse.id_token, 'candidate');
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      const msg = err.message || 'Lưu sở thích thất bại';
      setError(msg);
      toast.error(msg);
    }
  };
  ...
  return (
    <div className="flex h-[100dvh] bg-white font-sans overflow-hidden">
      ...
      <div className="hidden lg:block h-[100dvh] bg-[#f8fafc]">
        <img src="/banner_tuyendung.png" alt="Đăng ký VinJobs" className="h-full w-auto object-contain block" />
      </div>
        ...
        <div className="w-full max-w-[420px] my-auto shrink-0 pb-8 mt-12 lg:mt-0">
          ...
          <div className="mb-8">
            ...
            <h2 className="text-[28px] font-bold text-gray-900 mb-2">
              {step === 'otp' ? 'Xác thực Email' : 'Đăng ký ứng viên'}
            </h2>
            <p className="text-gray-500 text-[15px]">
              {step === 'otp' 
                ? `Vui lòng nhập mã OTP gồm 6 chữ số vừa được gửi đến email ${email}` 
                : 'Khám phá hàng ngàn cơ hội việc làm ngay hôm nay.'}
            </p>
          </div>
          ...
          {success && (
            <div className="bg-green-50 text-green-600 p-3 rounded-md mb-6 text-sm flex items-center gap-2 border border-green-100">
              <span className="mi text-lg">check_circle</span> <span>{success}</span>
            </div>
          )}
          ...
          {step === 'form' ? (
            <div className="animate-fade-in">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-[13px] font-semibold text-gray-700 mb-1.5">Họ và tên <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[13px] font-semibold text-gray-700 mb-1.5">Số điện thoại <span className="text-gray-400 font-normal">(tùy chọn)</span></label>
                  <input
                    type="tel"
                    className="w-full h-10 px-3 rounded-md border border-gray-300 focus:border-[#3b5bdb] focus:ring-1 focus:ring-[#3b5bdb] outline-none text-sm transition-all"
                ...
                <div className="pt-2 flex items-start gap-2">
                  ...
                  <label htmlFor="terms" className="text-xs text-gray-500 leading-tight">
                    Tôi đồng ý với <Link to="/terms" className="text-[#3b5bdb] hover:underline">Điều khoản dịch vụ</Link> và <Link to="/privacy" className="text-[#3b5bdb] hover:underline">Chính sách bảo mật</Link> của VinJobs.
                  </label>
                </div>
                ...
                <div className="pt-2">
                  <button type="submit" disabled={loading || !captchaToken} className="w-full h-11 bg-[#bad1f8] hover:bg-[#a5c2f3] text-[#1c3c9c] font-semibold rounded-md transition-colors flex items-center justify-center disabled:opacity-50">
                    {loading ? <span className="w-5 h-5 border-2 border-[#1c3c9c] border-t-transparent rounded-full animate-spin"></span> : 'Tạo tài khoản'}
                  </button>
                </div>
              </form>
            </div>
            ...
            <div className="animate-fade-in">
              <form onSubmit={handleVerifyOTP} className="space-y-4">
                <div>
                  <label className="block text-[13px] font-semibold text-gray-700 mb-1.5">Mã OTP <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    className="w-full h-12 px-3 rounded-md border border-gray-300 focus:border-[#3b5bdb] focus:ring-1 focus:ring-[#3b5bdb] outline-none text-center text-lg tracking-[0.5em] font-bold transition-all"
                ...
                <div className="pt-4">
                  <button type="submit" disabled={loading || otp.length !== 6} className="w-full h-11 bg-[#bad1f8] hover:bg-[#a5c2f3] text-[#1c3c9c] font-semibold rounded-md transition-colors flex items-center justify-center disabled:opacity-50">
                    {loading ? <span className="w-5 h-5 border-2 border-[#1c3c9c] border-t-transparent rounded-full animate-spin"></span> : 'Xác thực & Đăng nhập'}
                  </button>
                </div>
              </form>
              <div className="text-center mt-6">
                <p className="text-[14px] text-gray-600">
                  Chưa nhận được mã?{' '}
                  <button type="button" onClick={handleResendOTP} className="text-[#3b5bdb] font-bold hover:underline">
                    Gửi lại mã
                  </button>
                </p>
              </div>
          ...
          {step === 'form' && (
            <div className="text-center mt-8">
              <p className="text-[14px] text-gray-600 mb-4">
                Đã có tài khoản? <Link to="/login" className="text-[#3b5bdb] font-bold hover:underline">Đăng nhập</Link>
              </p>
