...
export default function EditJobPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [provs, catsRes, jobRes] = await Promise.all([
        ...
        if (parts.length === 4) {
          ...
          exact_address = job.location;
        }

        let province_code, district_code, ward_code;
        const matchedProv = provs.find(p => p.name === province_name);
        if (matchedProv) {
          province_code = matchedProv.code;
          const dists = await LocationService.getDistricts(province_code);
          setDistricts(dists);
          const matchedDist = dists.find(d => d.name === district_name);
          if (matchedDist) {
            district_code = matchedDist.code;
            const ws = await LocationService.getWards(district_code);
            setWards(ws);
            const matchedWard = ws.find(w => w.name === ward_name);
            if (matchedWard) {
        ...
        form.setFieldsValue({
          ...
          working_hours: job.working_hours?.length === 2 ? [dayjs(job.working_hours[0], 'HH:mm'), dayjs(job.working_hours[1], 'HH:mm')] : [],
  ...
  return (
    <EmployerLayout>
      ...
      <>
      <div className="mb-6">
        <Title level={3} style={{ margin: 0 }}>Cập nhật tin tuyển dụng</Title>
        <Text type="secondary">Chỉnh sửa tin đăng chuyên nghiệp</Text>
      </div>
      ...
      >
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-xl">
          ...
          {currentStep < STEPS.length - 1 ? (
            ...
            <Button type="primary" loading={saving} onClick={handleFinish} className="w-40 bg-green-600 hover:bg-green-700 font-bold">CẬP NHẬT TIN</Button>
          )}
        </div>
      </Form>
      </>
